...
from .metadata_inspector import build_columns_sql, build_primary_keys_sql, build_table_comment_sql, rows_to_table_metadata
...
class OracleClient:
    ...
    def fetch_ddl_with_comments(self, table_name: str, owner: str | None = None) -> str:
        return self.fetch_ddl(table_name, owner=owner)
    ...
    def inspect_table(self, table_name: str, owner: str | None = None) -> TableMetadata:
        ...
        with self._connect() as connection:
            ...
            table_comment_rows = _fetch_dicts(connection, build_table_comment_sql(), {"owner": bind_owner, "table_name": bind_table})
        if not column_rows:
            raise ValueError(f"Table not found: {bind_owner}.{bind_table}")
        primary_keys = {row["COLUMN_NAME"] for row in pk_rows}
        table_comment = table_comment_rows[0].get("COMMENTS") if table_comment_rows else None
        return rows_to_table_metadata(bind_table, column_rows, primary_keys, owner=bind_owner, table_comment=table_comment)
